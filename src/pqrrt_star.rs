...
impl PQRRTStarParams {
    ...
    pub fn from_file(filename: &str) -> Self {
        let cfg = Config::builder()
            ...
            .build()
            .unwrap()
            .try_deserialize::<PQRRTStarParams>()
            .unwrap();
        cfg
    }
    ...
    pub fn grow_towards_goal(
        ...
        while self.num_nodes < self.params.max_nodes && self.num_iter < self.params.max_iter {
            ...
            if self.is_collision_free(&xs_array) && t_new > self.params.min_steering_time {
                ...
                let mut Z_union = Z_near.clone();
                Z_union.extend(Z_parents);
                let (z_new_, z_parent) = self.choose_parent(&z_new, &z_nearest, &Z_union)?;
                z_new = z_new_;
...
impl PQRRTStar {
    ...
    pub fn rewire(&mut self, z_new: &RRTNode, Z_near: &Vec<RRTNode>) -> PyResult<()> {
        ...
        for z_near in Z_near.iter() {
            ...
            for z_from in Z_new_ancestry.iter() {
                if z_near_id == z_from.clone().id.unwrap() {
                    continue;
                }
    ...
    fn nearest_neighbors(&self, z_new: &RRTNode) -> PyResult<Vec<RRTNode>> {
        ...
        // Extract all nodes within the ball radius, greater than min node dist unless the
        // angle difference between the two nodes is small enough
        let mut Z_near = self
            .rtree
    ...
    pub fn ancestor(&self, z: &RRTNode, d: u64) -> PyResult<RRTNode> {
        ...
        let mut z_parent = z.clone();
        for i in 0..d {
            let z_parent_id = match self.get_parent_id(&z_parent) {
                Ok(id) => id,
                Err(_) => {
                    if i > 0 {
                        // If we have already found an ancestor, return that one
                        return Ok(z_parent.clone());
                    };
                    // If we have not found an ancestor, return error
                    return Err(PyErr::new::<pyo3::exceptions::PyException, _>(
    ...
    pub fn ancestry(&self, Z: &Vec<RRTNode>) -> PyResult<Vec<RRTNode>> {
        ...
        for z in Z {
            for l in 0..self.params.max_ancestry_level {
                let z_ancestor_l = match self.ancestor(&z, l + 1) {
                    Ok(z) => z,
                    Err(_) => continue,
                };
                let mut id_already_in_set = Z_parents.iter().any(|z_p: &RRTNode| {
                    z_p.clone().id.unwrap() == z_ancestor_l.clone().id.unwrap()
                });
                id_already_in_set = id_already_in_set
                    || Z.iter()
                        .any(|z_| z_.clone().id.unwrap() == z_ancestor_l.id.clone().unwrap());
        ...
        // println!("Num ancestry nodes: {}", Z_parents.len());
        Ok(Z_parents)
    }
    ...
    pub fn adjust_sample(&mut self, z_rand: &RRTNode) -> PyResult<RRTNode> {
        ...
        for _n in 0..self.params.max_sample_adjustments {
            ...
            total_distance += self.params.lambda_sample_adjustment * F_att.normalize().norm();
        }
    ...
    pub fn sample(&mut self) -> PyResult<RRTNode> {
        ...
        let map_bbox = utils::bbox_from_corner_points(&p_start, &p_goal, 500.0, 500.0);
...
mod tests {
    ...
    fn test_grow_towards_goal() -> PyResult<()> {
        ...
        Python::with_gil(|py| -> PyResult<()> {
            ...
            let result = rrt.grow_towards_goal(xs_start_py, 4.0, false, false, py)?;
