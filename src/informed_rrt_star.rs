...
impl InformedRRTStar {
    ...
    #[allow(non_snake_case)]
    fn set_speed_reference(&mut self, U_d: f32) -> PyResult<()> {
        Ok(self.U_d = U_d)
    }
...
impl InformedRRTStar {
    ...
    pub fn rewire(&mut self, z_new: &RRTNode, Z_near: &Vec<RRTNode>) -> PyResult<()> {
        ...
        for z_near in Z_near.iter() {
            ...
            let (xs_array, u_array, _, t_new, reached) = self.steer(
                &z_new.clone(),
                &z_near.clone(),
                3.0 * self.params.max_steering_time,
                5.0,
            )?;
    ...
    pub fn sample(&mut self) -> PyResult<RRTNode> {
        ...
        loop {
            let p_rand = if self.c_best != f32::INFINITY {
...
mod tests {
    ...
    fn test_sample() -> PyResult<()> {
        let mut rrt = InformedRRTStar::py_new(
