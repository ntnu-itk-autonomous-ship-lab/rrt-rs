...
@dataclass
class RRTConfig:
    params: RRTParams = field(default_factory=lambda: RRTParams())
...
class RRTPlannerParams:
    ...
    def from_dict(cls, config_dict: dict):
        config = RRTPlannerParams(
            los=guidances.LOSGuidanceParams.from_dict(config_dict["los"]),
            rrt=RRTConfig.from_dict(config_dict["rrt"]),
        )
        return config
...
class RRT(ci.ICOLAV):
        ...
        **kwargs,
        ...
        if not self._initialized:
            ...
            if enc is not None:
                ...
                # plotters.plot_trajectory(self._rrt_waypoints, enc, "orange")
    ...
    def reset(self):
        """Resets the RRT to its initial state."""
        self._rrt_inputs: np.ndarray = np.empty(3)
        self._rrt_trajectory: np.ndarray = np.empty(6)
        self._rrt_waypoints: np.ndarray = np.empty(3)
        self._geometry_tree: strtree.STRtree = strtree.STRtree([])
