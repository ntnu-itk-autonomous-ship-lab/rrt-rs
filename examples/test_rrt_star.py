        ...
        if not self._initialized:
            ...
            for aa in range(0, n_mc):
                ...
                if aa == 0 and enc is not None:
                    ...
                    enc.save_image(name="rrt_star_larger_planning_case", path=path, dpi=100, extension="png")
            ...
            print(
                f"cost: {costs_arr.mean():.2f} +/- {costs_arr.std():.2f} | cost (min, max): {costs_arr.min():.2f}, {costs_arr.max():.2f}"
            )
            results = {
            ...
            pd.DataFrame(results).to_json("rrt_star_results_larger_planning_case.json")
